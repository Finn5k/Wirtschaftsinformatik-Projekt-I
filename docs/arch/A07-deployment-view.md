...
**Konfiguration.** Die SPA erhält die Adresse des Supabase-Projekts und den öffentlichen Projektschlüssel als Deployment-Umgebungsvariablen `VITE_SUPABASE_URL` und `VITE_SUPABASE_PUBLISHABLE_KEY` (Muster in [`.env.example`](../../.env.example)). Vite ersetzt sie beim Bauen, nicht zur Laufzeit — sie müssen also vor dem Build gesetzt sein, und ein bestehendes Deployment übernimmt geänderte Werte erst nach einem erneuten Build. Fehlen sie, startet die Anwendung nicht und zeigt stattdessen einen Konfigurationshinweis (`src/main.tsx`); ohne NB-03 ist LocalCourt fachlich nicht nutzbar ([S1.4](../spec/S1-nachbarsysteme.md#s14-nb-03--supabase-postgrest)). Beide Werte sind öffentlich ([S1.1](../spec/S1-nachbarsysteme.md#s11-konventionen)); der geheime Service-Role-Key gehört weder in die Vercel-Clientvariablen noch ins Repository.
