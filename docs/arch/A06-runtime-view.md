...
- **Die Prüfung liegt in der RPC, nicht im Client.** `check_in` prüft Teilnahme, Merkmal und Zeitfenster gegen die Serverzeit und setzt den Status atomar ([F3 AF-02](../spec/F3-anwendungsfunktionen.md#af-02--check-in-validierung)); die Dialogseite reicht das Merkmal nur unverändert über die Service-Schicht an NB-03 durch — beim Deep-Link ohne weitere Nutzeraktion, sobald Session und Teilnahme geladen sind ([B1.4.6](../spec/B1-dialogspezifikation.md#b146-dlg-06--check-in) *QR-Einstieg*).
- **Das Sperrbild ist keine zweite Fachprüfung.** Ist die Session laut `v_session` nicht `active` oder liefert die RLS keine eigene Teilnahme, bietet `CheckInPage` das Formular gar nicht erst an, sondern zeigt den Zustand *Gesperrt* mit dem Ergebnistext aus B1.4.6. Die Werte stammen aus denselben Datenbankfunktionen, die `check_in` verwendet; entschieden wird weiterhin nur in der RPC.
...
- **Die Court-Anlage gehört mit in die RPC.** `courtService` liest nur die vorhandenen Courts zur Auswahl. Ein neu erfasster Sportort wird nicht vorab angelegt, sondern als Feldsatz an `create_session` übergeben und dort gemeinsam mit der Session erzeugt. Ein getrennter Schreibzugriff hinterließe bei einem Fehlschlag der anschließenden Session-Erstellung einen verwaisten Court — genau den unvollständigen Zwischenstand, den [ADR-001](A09-architecture-decisions.md#91-adr-001--atomare-fachoperationen-über-postgresql-funktionen-rpc-statt-clientseitiger-prüfung) ausschließt. Die Trennung, die [S1.4](../spec/S1-nachbarsysteme.md#s14-nb-03--supabase-postgrest) für `courtAnlegen` vorsah, ist damit aufgehoben.
- **`create_session` ist atomar für Court, Session und Organisator-Teilnahme.** Die RPC legt bei Bedarf den Court, dann die Session mit erzeugter PIN ([F3 AF-04](../spec/F3-anwendungsfunktionen.md#af-04--pin--und-qr-code-erzeugung)) sowie den `organizer`- und `participant`-Eintrag in einem Schritt an — belegt durch [N2.2](../spec/N2-querschnittskonzepte.md#n22-row-level-security-rls) („kein Schreibzugriff außer über die Erstellungs-RPC, die `organizer`- und `participant`-Eintrag atomar mit der Session anlegt") und [S1.4](../spec/S1-nachbarsysteme.md#s14-nb-03--supabase-postgrest) (`create_session` als eine der drei atomaren RPCs).
