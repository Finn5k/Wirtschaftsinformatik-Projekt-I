...
**Stand:** N1 formuliert Anforderungen an das MVP, keine bereits erreichten Zusagen. Wie weit eine Anforderung umgesetzt ist, steht in [A08](../arch/A08-crosscutting-concepts.md); geprüft wird manuell ([N1.3](#n13-bewusst-nicht-verfolgte-qualitätsziele)).
...
| Akzeptanzkriterien | **Given** eine Session mit genau einem freien Platz,<br>**When** zwei Nutzer nahezu gleichzeitig beitreten,<br>**Then** wird genau einer bestätigt und der andere erhält `SESSION_FULL`.<br><br>**Given** eine bereits eingecheckte Teilnahme,<br>**When** der Check-in wiederholt wird,<br>**Then** bleibt der Zeitpunkt unverändert und die Antwort lautet `ALREADY_CHECKED_IN`.<br><br>**Given** einen nicht erreichbaren Kartendienst,<br>**When** der Nutzer die Suche öffnet,<br>**Then** bleibt die Listenansicht bedienbar. |
...
| Akzeptanzkriterien | **Given** einen Viewport von höchstens 768 px,<br>**When** ein Nutzer einen der acht Dialoge öffnet,<br>**Then** ist der gesamte Inhalt ohne horizontales Scrollen erreichbar und jede Muss-Aktion bedienbar.<br><br>**Given** einen Check-in-Deep-Link auf einem Telefon,<br>**When** er aus einer Kamera-App geöffnet wird,<br>**Then** ist der Ablauf ohne Zoomen durchführbar. |
...
| Abschnitt | Inhalt |
|---|---|
| Beschreibung | Geschützte Aktionen — Beitritt, Session-Erstellung, Check-in und Profilverwaltung — stehen nur angemeldeten Nutzern offen. Von fremden Profilen sind ausschließlich `display_name` und optional `avatar_url` sichtbar; die Teilnehmerliste einer Session sieht nur ihr Organisator, andere Nutzer sehen die Belegung und ihre eigene Teilnahme ([N2.2](N2-querschnittskonzepte.md#n22-row-level-security-rls)). PIN und QR-Code erscheinen nur im Organisator-Zustand von DLG-04. Geheime Schlüssel liegen weder im Frontend-Bundle noch im Repository. |
...
| Akzeptanzkriterien | **Given** einen nicht angemeldeten Nutzer,<br>**When** er eine geschützte Aktion auslöst,<br>**Then** wird er zu DLG-01 geleitet und die Aktion wird nicht ausgeführt.<br><br>**Given** einen angemeldeten Nutzer, der eine Session nicht organisiert,<br>**When** er die Session öffnet,<br>**Then** sieht er keine fremden Teilnahmen — nur die Belegung und, falls beigetreten, seine eigene.<br><br>**Given** den Organisator-Zustand von DLG-04,<br>**When** die Teilnehmerliste angezeigt wird,<br>**Then** erscheinen je Teilnehmer ausschließlich Anzeigename, optionales Profilbild und Check-in-Status.<br><br>**Given** den Teilnehmer-Zustand von DLG-04,<br>**When** ein Nutzer die Session öffnet,<br>**Then** ist die PIN nicht sichtbar. |
| Prüfmethode | Geschützte Routen ohne Anmeldung aufrufen; eine Session als Teilnehmer und als Organisator öffnen und die sichtbaren Teilnahmen sowie die Felder der Teilnehmerliste gegen D1.4 abgleichen; Repository und Git-Verlauf auf Schlüsselmuster und `.env`-Dateien durchsuchen. |
