...
|---|---|---|---|
| `profile` | `user_id` (`uuid`, referenziert Supabase-Auth-Nutzer mit `ON DELETE CASCADE`, kein eigener Default) | — | `display_name` `NOT NULL`. |
...
| `session` | `session_id` (`uuid`) | `organizer_id → profile.user_id` (`ON DELETE CASCADE`), `sport_id → sport.sport_id`, `court_id → court.court_id` (jeweils `NOT NULL`, für Sport/Court `ON DELETE RESTRICT`) | `CHECK (duration_min >= 1)`, `CHECK (pin ~ '^[0-9]{4}$')`, `CHECK (max_participants >= 1)`. Die Löschwirkung auf organisierte Sessions dient ausschließlich dem administrativen DSGVO-Prozess aus N2.15 und ist keine Session-Löschfunktion des MVP. |
| `participant` | `participant_id` (`uuid`) | `session_id → session.session_id`, `user_id → profile.user_id` (beide `NOT NULL`, jeweils `ON DELETE CASCADE`) | `UNIQUE (session_id, user_id)` — technische Umsetzung der D1.4-Invariante „höchstens eine Teilnahme je (session_id, user_id)" und Grundlage der Atomarität in [N2.4](#n24-atomarität-des-beitritts-af-01). `CHECK ((status = 'checked_in') = (checked_in_at IS NOT NULL))` setzt die Check-in-Kopplung aus D1.4 technisch um. |
...
## N2.15 DSGVO-Auskunft und Löschung

Auskunfts- und Löschanfragen werden im MVP nicht über einen zusätzlichen Dialog oder Use Case, sondern über die in der Datenschutzerklärung genannte Kontaktadresse bearbeitet. Der Nutzer stellt die Anfrage mit der im Konto hinterlegten E-Mail-Adresse; vor der Bearbeitung wird seine Identität geprüft.

Die Auskunft umfasst die dem Nutzer zugeordneten Profil-, Sportpräferenz-, Session- und Teilnahmedaten. Bei bestätigter Löschung entfernt ein Administrator das Supabase-Auth-Konto. Über die in N2.3 festgelegten Fremdschlüssel werden anschließend das Profil, Sportpräferenzen, Teilnahmen sowie vom Nutzer organisierte Sessions und deren abhängige Teilnahmen gelöscht. Bei Courts wird `created_by` auf `NULL` gesetzt, damit nicht personenbezogene Sportortdaten erhalten bleiben. Technisch notwendige Nachweise dürfen nur ohne Personenbezug fortbestehen. Eine Self-Service-Löschung ist nicht Teil des MVP.

### Verbleibender betrieblicher Prüfpunkt
...
Nicht mehr offen: Feldlängen, maximale Session-Dauer und Check-in-Zeittoleranz sind in [N1.7](N1-nichtfunktionale-anforderungen.md#n17-bewusst-nicht-festgelegte-qualitätsanforderungen) bewusst nicht festgelegt. Sichtbare Profilfelder stehen in [D1.4](D1-datenmodell.md#d14-entitätstypen-im-detail) und B1 DLG-04. Das akzeptierte Dublettenverhalten steht in B1 DLG-05 und F2 UC-10.
