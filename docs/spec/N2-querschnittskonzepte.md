...
|---|---|---|
| `session` | Lesbar auch ohne Anmeldung: [UC-02](F2-anwendungsfaelle.md#uc-02--session-suchen) setzt für die Suche keine Anmeldung voraus, und [B1.5.2](B1-dialogspezifikation.md#b152-weiterleitung-nicht-angemeldeter-nutzer) zählt Suche und Detailansicht nicht zu den geschützten Aktionen. Kein Schreibzugriff außer über die Erstellungs-RPC. Die PIN ist von diesem Leserecht ausgenommen (nächste Zeile). | UC-02, UC-06 |
| `session.pin` (Spalten-Ebene) | Nur sichtbar für Nutzer mit `organizer`-Eintrag (`organizer.user_id = auth.uid()`) für diese Session. Teilnehmer, auch bestätigte oder eingecheckte, erhalten die PIN nicht über das System, sondern vom Organisator vor Ort ([UC-09](F2-anwendungsfaelle.md#uc-09--check-in-per-pin-durchführen)); die Prüfung der eingegebenen PIN bleibt bei AF-02. | AF-02, AF-04 |
| `organizer` | Lesbar auch ohne Anmeldung, wie `session` — die Anzeige „Organisator" gehört zur Detailansicht (UC-03), die B1.5.2 nicht schützt. Kein Schreibzugriff außer über die Erstellungs-RPC (`create_session`), die `organizer`- und `participant`-Eintrag atomar mit der Session anlegt. | UC-03, UC-06 |
...
| `profile` | Basisfelder (`display_name`, `avatar_url`) auch ohne Anmeldung lesbar: [B1 DLG-04](B1-dialogspezifikation.md#b144-dlg-04--session-detail) führt den Organisator als Anzeigefeld, und [B1.2](B1-dialogspezifikation.md#b12-dialoglandkarte) macht diesen Dialog ohne Anmeldung einsehbar. `city` ist davon ausgenommen und nur für `user_id = auth.uid()` lesbar; `display_name` und `city` nur für `user_id = auth.uid()` schreibbar. `avatar_url` bleibt im MVP unverändert. | UC-12, D1.4 „Datenschutz" |
| `court`, `sport`, `sport_preference` | `court`/`sport` lesbar für alle. Kein direkter Schreibzugriff auf `court`: Ein neu erfasster Sportort entsteht ausschließlich innerhalb der Erstellungs-RPC `create_session` ([S1.4](S1-nachbarsysteme.md#s14-nb-03--supabase-postgrest)), die `created_by = auth.uid()` setzt (UC-10). `sport_preference` nur für den eigenen `user_id` schreibbar. | UC-10, UC-12 |
