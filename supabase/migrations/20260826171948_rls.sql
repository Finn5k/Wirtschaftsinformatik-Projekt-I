...
--
-- Setzt die Policy-Tabelle aus N2.2 um. Autorisierung liegt vollstaendig auf
-- Datenbankebene, weil TECH-03 eine eigene Backend-Schicht ausschliesst und der
...
-- fuer alle ANGEMELDETEN Nutzer". Das widerspricht drei anderen Bausteinen:
--   - UC-02 Vorbedingung: "Anmeldung ist fuer die Suche nicht zwingend erforderlich"
--   - B1.2: "Suche und Detailansicht (DLG-02, DLG-03, DLG-04) sind ohne Anmeldung nutzbar"
--   - B1.5.2: geschuetzt sind nur Beitreten, Erstellen, Check-in, Meine Sessions, Profil
-- Umgesetzt ist die Mehrheitslesart (UC-02/B1): Sessions, Courts und Sportarten
-- sind auch unangemeldet lesbar. Personenbezogene Daten bleiben davon unberuehrt -
-- profile und participant sind weiterhin nur angemeldet zugaenglich (N1-QA-03).
...
-- ------------------------------------------- Standardrechte zuruecknehmen
-- Supabase vergibt auf public schema-weite Rechte an anon/authenticated. Fuer
-- ein belastbares Rechtebild werden sie zurueckgenommen und unten gezielt neu
...
-- =============================================================== sport
-- N2.2: lesbar fuer alle. Stammdaten - kein Schreibzugriff fuer Endnutzer.
...
-- =============================================================== court
-- N2.2: lesbar fuer alle; Erstellung durch angemeldete Nutzer mit
...
-- ============================================================= session
-- N2.2: lesbar; kein Schreibzugriff ausser ueber create_session.
-- Deshalb wird ausschliesslich SELECT erteilt - und zwar spaltenweise ohne pin.
...
-- security_invoker = on sorgt dafuer, dass die Policy oben auch hier greift.
...
-- kein Schreibzugriff ausser ueber create_session.
...
-- ========================================================= participant
-- N2.2: lesbar fuer den Organisator der Session (Teilnehmerliste, UC-07) und
-- fuer den Nutzer selbst (UC-05, UC-11). Schreibzugriff ausschliesslich ueber
...
-- ============================================================= profile
-- N2.2 und D1.4 "Datenschutz": Fuer andere Nutzer sind ausschliesslich
...
-- teilweise" ausdruecken. Deshalb: die Basistabelle gibt nur die eigene Zeile
-- frei, fremde Basisfelder laufen ueber v_profile_public.
...
-- avatar_url bleibt im MVP unveraendert (N2.2) und ist deshalb nicht im
...
-- kontrolliert weniger Spalten freizugeben, als die Zeilenpolicy oben zulaesst.
...
comment on view public.v_profile_public is
  'D1.4 Datenschutz - fuer andere Nutzer sichtbare Profilfelder (display_name, avatar_url).';

-- ==================================================== sport_preference
-- N2.2: nur fuer den eigenen user_id schreibbar (UC-12,
...
-- ========================================================== PIN-Zugriff
-- N2.2 Spalten-Policy: die PIN ist nur fuer den Organisator und fuer
-- bestaetigte Teilnehmer sichtbar. Das SELECT-GRANT auf session enthaelt die
-- Spalte nicht; dieser gekapselte Zugriff ist der einzige Weg.
--
-- Rueckgabe NULL statt Fehler: N2.2 formuliert eine Sichtbarkeitsregel, und
-- N2.2 warnt ausdruecklich davor, eine technische Zugriffsablehnung mit einem
...
comment on function public.session_pin(uuid) is
  'N2.2 Spalten-Policy - PIN nur fuer Organisator und bestaetigte Teilnehmer, sonst NULL.';

-- ------------------------------------------------- Ausfuehrrechte der RPCs
-- Die drei atomaren Fachoperationen pruefen die Anmeldung selbst und geben
-- NOT_AUTHENTICATED zurueck (F3 AF-01/AF-02), statt am fehlenden Recht zu
-- scheitern - deshalb ist auch anon ausfuehrungsberechtigt.
